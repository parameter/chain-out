...
async function sendUserNotification({ forUserId, eventName, payload, expoPush, localNotification }) {
  ...
  if (expoPush) {
    ...
    try {
      ...
      const expoPushToken = await expoPushTokensCollection.findOne({ userId: new ObjectId(forUserId) });
        ...
        },
        {
          $lookup: {
            ...
            pipeline: [
              ...
              { $project: { _id: 0, username: 1, fname: 1, sname: 1, profileImage: 1, isPremium: 1 } },
              { $limit: 1 }
            ],
            as: '_lbUser'
          }
...
router.get('/friends', requireAuth, async (req, res) => {
  try {
    ...
    const friendBlocksCollection = db.collection('friend-blocks');
    const friendBlocks = await friendBlocksCollection.find({ $or: [{ user: currentUserId, blockedBy: req.user._id.toString() }, { blockedBy: currentUserId, user: req.user._id.toString() }] }).toArray();
      ...
      .aggregate([
        {
          $match: {
            status: 'accepted',
            $or: [{ to: currentUserId, from: req.user._id.toString() }, { from: currentUserId, to: req.user._id.toString() }],
