...
const updateBadgeProgress = async (userId, progressData, badgeDefinitions) => {
  ...
  if (existingProgress) {
    ...
    // Start with existing tierProgress array
    let tierProgress = existingProgress.tierProgress || [];
    
    // Check for new tier achievements and add them to tierProgress
    if (newTier > existingProgress.currentTier) {
      // Add new tier achievements to the array
      for (let tierIndex = existingProgress.currentTier + 1; tierIndex <= newTier; tierIndex++) {
        tierProgress.push({
          tierIndex: tierIndex,
          achieved: true,
      ...
      // Calculate new tiers for return value
    ...
    // Create tierProgress array with all achieved tiers
    const tierProgress = [];
    if (newTier >= 0) {
      for (let tierIndex = 0; tierIndex <= newTier; tierIndex++) {
        tierProgress.push({
          tierIndex: tierIndex,
          achieved: true,
          achievedDate: new Date(),
...
const checkTierAchievement = async (userId, badgeId, targetTierIndex) => {
  ...
  try {
    ...
    if (userProgress.tierProgress && userProgress.tierProgress.length > 0) {
      // Find the tier info for the target tier
      tierInfo = userProgress.tierProgress.find(tier => tier.tierIndex === targetTierIndex);
      if (!tierInfo && achieved) {
        // If target tier is achieved but not in tierProgress, get the highest tier info
        const highestTier = userProgress.tierProgress.reduce((max, tier) => 
          tier.tierIndex > max.tierIndex ? tier : max
        );
        tierInfo = highestTier;
      }
    }
...
const getUserBadgeTierAchievements = async (userId, badgeId) => {
  ...
  try {
    ...
    // Calculate which tiers have been achieved using tierProgress history
    const allTiersAchieved = [];
    for (let i = 0; i < badgeDef.tierThresholds.length; i++) {
      if (userProgress.currentTier >= i) {
        // Find the tier info from tierProgress array
        const tierInfo = userProgress.tierProgress?.find(tier => tier.tierIndex === i);
        allTiersAchieved.push({
          tierIndex: i,
          threshold: badgeDef.tierThresholds[i],
          achieved: true,
          achievedDate: tierInfo?.achievedDate || userProgress.lastUpdated,
          progress: tierInfo?.progress || userProgress.totalProgress
        });
      }
    }
