...
const getUserAllBadges = async (userId) => {
  ...
  // Use aggregation because we need to JOIN two collections (MongoDB doesn't support SQL-style JOINs)
  // Potential duplicate sources:
  // 1. Multiple userBadgeProgress docs for same userId+badgeId (shouldn't happen but possible)
  // 2. Multiple badgeDefinitions with same 'id' field (shouldn't happen but possible)
  // 3. $unwind creates one doc per matched badgeDef (if multiple badgeDefs match)
  const result = await badgesCollection.aggregate([
    { $match: { userId: userId } },
    // Group by badgeId to ensure only one progress doc per badge (handles case 1)
    {
      $group: {
        _id: "$badgeId",
    ...
    // Use $arrayElemAt instead of $unwind to prevent duplicates from multiple badgeDef matches
    // This takes only the first matching badge definition
    {
      $addFields: {
