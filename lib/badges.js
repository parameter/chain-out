...
const evaluateAchievementStreak = ({ relevantResults, scope, minimum, expectedResult, expectedZone, holesByNumber }) => {
  if (!Array.isArray(relevantResults) || relevantResults.length === 0) return false;
  const min = Number(minimum);
  const minRequired = Number.isFinite(min) && min > 0 ? Math.floor(min) : 1;
  const safeScope = String(scope || 'same-round');
  if (safeScope === 'multi-round') {
    // "multi-round" is validated through persisted progress, so a single round match is enough here.
    return relevantResults.some((r) => {
  ...
  if (safeScope === 'in-a-row-same-round') {
    let bestRun = 0;
    let currentRun = 0;
    ...
    return bestRun >= minRequired;
  }

  const matchCount = sorted.filter(predicate).length;
...
const scorecardUsesScramble = (scorecard) => {
  if (!scorecard) return false;
  if (typeof scorecard.scramble === 'boolean') return scorecard.scramble;
  if (typeof scorecard.scramble === 'string') return scorecard.scramble.toLowerCase() === 'yes';
  return scorecard.mode === 'doubles' || scorecard.mode === 'scramble';
};

const searchForEarnedAchievements = async ({ scorecardId, results, courseId, layout, scorecard }) => {

  console.log('searchForEarnedAchievements', results, courseId, layout, scorecard);
  ...
  const achievementDefs = await achievementsCollection.find({ courseId: courseFilterId }).toArray();
  console.log('achievementDefs', achievementDefs);
  if (!achievementDefs.length) return [];
  ...
  (results || []).forEach((r) => {
    const playerId = r?.entityId ?? r?.playerId;
  ...
  const isScramble = scorecardUsesScramble(scorecard);
  const earnedPerPlayer = [];
  const progressOps = [];
  const scorecardIdObj = ObjectId.isValid(scorecardId) ? new ObjectId(scorecardId) : scorecardId;

  for (const [playerId, playerResults] of playerResultsMap.entries()) {
    const earnedAchievements = [];
    const normalizedPlayerId = ObjectId.isValid(playerId) ? new ObjectId(playerId) : playerId;
    ...
    for (const ach of achievementDefs) {
      const selectedHoles = Array.isArray(ach.selectedHoles)
        ...
        : [];
      const relevantResults = selectedHoles.length
      ...
      const requiresVerified = Boolean(ach.verifiedOnly) || parseYesNo(ach.condition?.verifiedRound) === true;
      if (requiresVerified && !isVerifiedRound) continue;

      const requiresScramble = parseYesNo(ach.condition?.scramble);
      if (requiresScramble != null && requiresScramble !== isScramble) continue;
      ...
      const streakScope = ach?.streak?.scope || 'same-round';
      const streakSatisfied = evaluateAchievementStreak({
        relevantResults,
        scope: streakScope,
        minimum: Number.isFinite(streakMinimum) && streakMinimum > 0 ? streakMinimum : 1,
        expectedResult,
        expectedZone,
        holesByNumber
      });
      if (!streakSatisfied) continue;
    ...
    console.log('earnedAchievements', earnedAchievements);
