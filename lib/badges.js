...
const searchForEarnedBadges = async (scorecardInput) => {
  ...
  const normalizedLayout = {
    holes: holes.map((h, index) => ({
      holeNumber: h.number !== undefined ? h.number : (h.holeNumber !== undefined ? h.holeNumber : index + 1),
  ...
  for (const playerIdStr of uniquePlayerIds) {
    ...
    // Get all user's badge progress once for efficiency
    const userBadgeProgress = await progressCollection.find({
      userId: playerId
    }).toArray();
    
    // Create a map for quick lookup of completed courses per badge
    const completedCoursesMap = {};
    userBadgeProgress.forEach(progress => {
      if (progress.completedCourses) {
        completedCoursesMap[progress.badgeId] = progress.completedCourses;
      }
    });
    ...
    for (const badgeDef of badgeDefinitions) {
      try {
        // For badges that track unique courses, check if this course was already counted
        if (badgeDef.trackUniqueCourses && completedCoursesMap[badgeDef.id] && 
            completedCoursesMap[badgeDef.id].includes(courseId)) {
          console.log(`Course ${courseId} already counted for badge ${badgeDef.name}, skipping`);
          continue; // Skip this badge for this course
        }
...
const updateBadgeProgress = async (userId, progressData, badgeDefinitions, courseId, layoutId) => {
  ...
  console.log('updateBadgeProgress progressData: ', progressData, { userId, progressData, badgeDefinitions, courseId, layoutId });
  ...
  if (existingProgress) {
    console.log('updateBadgeProgress 1:');
    ...
    // Add course to completed courses if this badge tracks unique courses
    ...
    console.log('updateBadgeProgress 2:');
    // Create new progress
...
const getUserAllBadges = async (userId) => {
  ...
  const result = await badgesCollection.find({ userId: userId }).toArray();
  return result;
};
