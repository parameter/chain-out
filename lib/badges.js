 ...
 */
function buildPlayersXpBreakdownEntry(entityId, roundXPResult, bonuses, badgeObjectsWithXP) {
  ...
  const tierAmounts = XP_TIER.map(() => 0);
  for (const bo of badgeObjectsWithXP || []) {
    const grant = Number(bo.xp) || 0;
    if (grant <= 0) continue;
    const tierIdx = Math.min(Math.max(Number(bo.currentTier) || 0, 0), XP_TIER.length - 1);
    tierAmounts[tierIdx] += grant;
  }
...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {  
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    let totalXP = 0;
    let xpBreakdownRow = buildPlayersXpBreakdownEntry(playerId, {
      baseXP: 0,
      hotStreakXP: 0,
      cleanRound: false,
      noOB: true,
    ...
    try {
      ...
      xpBreakdownRow = buildPlayersXpBreakdownEntry(playerId, roundXPResult, bonuses, badgeObjectsWithXP);
    } catch (xpErr) {
