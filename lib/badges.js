...
const updateBadgeProgress = async (userId, progressData, badgeDefinitions, courseId, layoutId, scorecardId) => {
  ...
  if (existingProgress) {
    ...
    if (badgeDef && badgeDef.trackTierThresholdZync) {
      var updatedTotal = progressData.progress;
      var newTier = calculateCurrentTier(updatedTotal, badgeDef.tierThresholds);
    } else {
      var updatedTotal = existingProgress.totalProgress + progressData.progress;
      var newTier = calculateCurrentTier(updatedTotal, badgeDef.tierThresholds);
    }
    ...
    if (badgeDef && badgeDef.trackTierThresholdZync) {
      updateOps.$addToSet = { trackedThresholds: newTier };
    }
    ...
    newProgress = {
      ...
      trackedThresholds: (badgeDef && badgeDef.trackTierThresholdZync) ? [newTier] : [],
      tierProgress: [{
        tierIndex: newTier,
        achieved: true,
        achievedDate: new Date(),
