...
const XP_MULTIPLIER = {
  /** Per consecutive birdie during the round (not an end-of-round %). */
  hotStreakPerBirdie: 1.2,
  /** End-of-round % bonuses; each applies to score + hot-streak + flat bonuses + badge XP. */
  weeklyStreak: 0.05,
  verifiedRound: 0.05,
...
function sumBadgesXpThisRound(badgeObjectsWithXP, uniqueBadgesEarnedThisRound = 0) {
  ...
  return badgesXpThisRound;
}

function computeRoundXpBeforeMultipliers(roundXPResult) {
  ...
  return { roundXP, circleTwoChampXp };
}

/** Score + hot-streak + flat bonuses + badge XP — the base for all end-of-round % multipliers. */
function computeXpBeforeEndOfRoundMultipliers(
  roundXPResult,
  badgeObjectsWithXP,
  uniqueBadgesEarnedThisRound = 0
) {
  const { roundXP, circleTwoChampXp } = computeRoundXpBeforeMultipliers(roundXPResult);
  const badgesXpThisRound = sumBadgesXpThisRound(badgeObjectsWithXP, uniqueBadgesEarnedThisRound);
  return {
    roundXP,
    circleTwoChampXp,
    badgesXpThisRound,
    xpBeforeMultiplier: roundXP + badgesXpThisRound
  };
}

/**
 * Marginal XP from end-of-round % multipliers (order: +verified, +weekly, +bogeyFreeStreak).
 * Each uses the same base: score XP, hot-streak bonus, flat bonuses, and badge XP.
 */
function splitEndOfRoundMultiplierPremium(xpBeforeMultiplier, bonuses) {
  ...
  const baseRounded = Math.round(xpBeforeMultiplier);
  const withV = Math.round(xpBeforeMultiplier * (1 + v));
  const withVW = Math.round(xpBeforeMultiplier * (1 + v + w));
  const withVWB = Math.round(xpBeforeMultiplier * (1 + v + w + b));
  return {
    verifiedRound: bonuses.verifiedRound ? withV - baseRounded : 0,
    weeklyStreak: bonuses.weeklyStreak ? withVW - Math.round(xpBeforeMultiplier * (1 + v)) : 0,
    bogeyFreeStreak: bonuses.bogeyFreeStreak ? withVWB - Math.round(xpBeforeMultiplier * (1 + v + w)) : 0
  };
}

function applyEndOfRoundMultipliers(xpBeforeMultiplier, bonuses) {
  let multiplier = 1;
  ...
  return {
    totalXP: Math.round(xpBeforeMultiplier * multiplier),
    multParts: splitEndOfRoundMultiplierPremium(xpBeforeMultiplier, bonuses)
  };
}
...
function buildPlayersXpBreakdownEntry(
  ...
  const { circleTwoChampXp, badgesXpThisRound, xpBeforeMultiplier } =
    computeXpBeforeEndOfRoundMultipliers(
      roundXPResult,
      badgeObjectsWithXP,
      uniqueBadgesEarnedThisRound
    );
  const { multParts } = applyEndOfRoundMultipliers(xpBeforeMultiplier, bonuses);
  ...
  for (const [userId, playerResults] of playerResultsMap.entries()) {
    ...
    for (const ach of dedupedAchievementDefs) {
      ...
      const selectedHoles = Array.isArray(ach.selectedHoles)
        ...
        : [];
      const selectedHolesMatch = ach.selectedHolesMatch === 'any' ? 'any' : 'all';
      ...
      if (selectedHoles.length) {
        ...
        const holesCovered = selectedHolesMatch === 'any'
          ? selectedHoles.some((n) => covered.has(n))
          : selectedHoles.every((n) => covered.has(n));
        if (!holesCovered) continue;
      }
      ...
      let roundContribution = streakScope === 'in-a-row-same-round'
        ? roundProgress.bestRun
        : roundProgress.matchCount;
      if (selectedHoles.length && selectedHolesMatch === 'any') {
        const hasMatch = roundProgress.matchCount > 0 || roundProgress.bestRun > 0;
        roundContribution = hasMatch ? 1 : 0;
      } else if (selectedHoles.length && selectedHolesMatch === 'all') {
        if (streakScope === 'in-a-row-same-round') {
          roundContribution = roundProgress.bestRun >= selectedHoles.length ? 1 : 0;
        } else {
          const matchedHoles = new Set(
            relevantResults
              ...
              .map((r) => Number(r.holeNumber))
          );
          roundContribution = selectedHoles.every((n) => matchedHoles.has(n)) ? 1 : 0;
        }
      }
...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    try {
      ...
      const { xpBeforeMultiplier } = computeXpBeforeEndOfRoundMultipliers(
        roundXPResult,
        badgeObjectsWithXP,
        uniqueBadgeDocsForPlayer.length
      );
      ({ totalXP } = applyEndOfRoundMultipliers(xpBeforeMultiplier, bonuses));
...
const getUserAllBadges = async (user) => {
