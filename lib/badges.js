...
const searchForEarnedBadges = async (scorecardInput) => {
  ...
  console.log('scorecardInput.results.length: ', scorecardInput.results.length);
  console.log('layout.latestVersion.holes.length: ', layout.latestVersion.holes.length);

  // error reporting for when the results are not the same as the holes
  if (holes.length === 0 || scorecard.results.length % holes.length !== 0) {
  ...
  for (const playerIdStr of uniquePlayerIds) {
    ...
    const progressBulkOps = updateBadgeProgress(playerId, progressForPlayer, badgeDefinitions, courseId, (layout._id || layout.id), scorecardInput._id);
