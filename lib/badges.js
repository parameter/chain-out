...
const defaultAchievementsModule = require('../data/default_achievements');
const default_achievements = Array.isArray(defaultAchievementsModule)
  ? defaultAchievementsModule
  : (Array.isArray(defaultAchievementsModule?.default) ? defaultAchievementsModule.default : []);
...
const getAchievementRoundProgress = ({ relevantResults, expectedResult, expectedZone, holesByNumber }) => {
...
const searchForEarnedAchievements = async ({ scorecardId, results, courseId, layout, scorecard }) => {
  ...
  const activeCourseAchievementsCollection = db.collection('active-course-achievements');
  const achievementProgressCollection = db.collection('userAchievementProgress');
  const courseFilterId = normalizeCourseId(courseId);

  const configuredDefaultsDoc = await activeCourseAchievementsCollection.findOne({ courseId: courseFilterId });
  ...
  console.log('achievementDefs', dedupedAchievementDefs);
  ...
  for (const [playerId, playerResults] of playerResultsMap.entries()) {
    ...
    for (const ach of dedupedAchievementDefs) {
      ...
      const nextProgress = alreadyCountedThisScorecard
        ? previousProgress
        : (streakScope === 'multi-round' ? previousProgress + roundContribution : roundContribution);
      const streakSatisfied = nextProgress >= minRequired;

      const shouldPersistProgress = !alreadyCountedThisScorecard && roundContribution > 0;
      if (shouldPersistProgress) {
        const progressUpdateSet = {
          title: ach.title,
          ...
          ...(streakSatisfied ? { won: true, lastEarnedAt: new Date(), wonAt: new Date() } : {})
        };
      ...
      const shouldEmitToScorecard = streakSatisfied || (String(streakScope) === 'multi-round' && nextProgress > 0);
      if (!shouldEmitToScorecard) continue;
      ...
      earnedAchievements.push({
        ...
        progress: nextProgress,
        minimum: minRequired,
        ...(streakSatisfied ? { won: true, completed: true } : {})
      });
    }
