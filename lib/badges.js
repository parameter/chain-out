...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {  
  ...
  // Precompute global statistics per badge using aggregates on userBadgeProgress
  ...
  try {
    // Tier-based percentages (for badges with tiers)
    ...
    // Unique badge earned percentage (including unique badges without tiers)
    const uniqueBadges = badgeDefinitions.filter(b => b.isUnique);
    const uniqueBadgeIds = uniqueBadges.map(b => b.id);

    if (uniqueBadgeIds.length > 0) {
      // Total distinct users that have any badge progress/earn
      const allUserIds = await progressCollection.distinct('userId');
      const totalUsersAll = allUserIds.length || 0;

      if (totalUsersAll > 0) {
        // For each unique badge, count distinct users that have a record
        ...
        const uniqueByBadgeId = {};
        uniqueAgg.forEach(doc => {
          uniqueByBadgeId[doc._id] = doc.earnedUsers || 0;
        });

        uniqueBadges.forEach(badgeDef => {
          const earnedUsers = uniqueByBadgeId[badgeDef.id] || 0;
          const earnedPercentage = totalUsersAll > 0 ? (earnedUsers / totalUsersAll) * 100 : 0;

          badgeTierStatsMap[badgeDef.id] = {
            ...(badgeTierStatsMap[badgeDef.id] || {}),
            badgeId: badgeDef.id,
            uniqueEarnedUsers: earnedUsers,
            uniqueEarnedPercentage: earnedPercentage
          };
        });
      }
    }
  } catch (error) {
    console.error('[searchForEarnedBadges] Failed to compute badge statistics:', error);
  }
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    const sortedProgressBadges = [...badgeObjects].sort((a, b) => (b.progress || 0) - (a.progress || 0));
    ...
    try {
      ...
      for (const bo of badgeObjects) {
        // Initialize per-badge XP
        bo.xpEarned = 0;

        // XP from tier increments for this badge
        if (bo.tierIncremented && bo.currentTier >= 0) {
          const xpForTier =
            XP_TIER[Math.min(bo.currentTier, XP_TIER.length - 1)] ||
            XP_TIER[XP_TIER.length - 1];

          badgeTierXP += xpForTier;
          bo.xpEarned += xpForTier;
        }
      }
...
const checkHistoricalBadges = async (db, playerId, historicalBadges, courseId, scorecardId = null) => {
  ...
  try {
    ...
    for (const facetKey of Object.keys(facets)) {
      ...
      if (badgeDef.isUnique && !aggConfig.asProgress) {
      ...
      } else if (badgeDef.isUnique && aggConfig.asProgress && hasTierThresholds) {
        ...
        const firstThreshold = badgeDef.tierThresholds[0];
...
const updateBadgeProgress = (userId, progressForPlayer, badgeDefinitions, courseId, layoutId, scorecardId, existingProgressMap = {}, badgeTierStatsMap = {}) => {
