...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout }) => {
  ...
  for (const playerIdStr of uniquePlayerIds) {
    ...
    let roundsOnCurrentCourse = 0;
    if (badgesRequiringHistory.length > 0) {

      const roundsResult = await scorecardsCollection.aggregate([
        {
          $match: {
            status: 'completed',
            courseId: courseIdObj,
            _id: { $ne: new ObjectId(scorecardId) },
            $or: [
              { creatorId: playerId },
      ...
      roundsOnCurrentCourse = roundsResult.length > 0 ? roundsResult[0].roundsCount : 0;
    }
      ...
      try {
        ...
        const conditionResult = executeBadgeCondition(
          ...
          { 
            ...
            roundsOnCurrentCourse: badgeDef.requiresHistoricalData ? roundsOnCurrentCourse : undefined,
...
const checkHistoricalBadges = async (db, playerId, historicalBadges, courseId) => {
  ...
  // We'll use $arrayElemAt to get the first document, then access the field
  const projectFields = {};
  for (const facetKey of Object.keys(facets)) {
    const valueField = badgeIdToConfig[facetKey].aggConfig.valueField;
    
    // Get first document from facet result array
    const firstDoc = { $arrayElemAt: [`$$${facetKey}`, 0] };
    
    // Access the value field - handle both simple and nested field paths
    if (valueField.includes('.')) {
      // For nested fields, use $getField (MongoDB 4.4+)
      const parts = valueField.split('.');
      let fieldAccess = firstDoc;
      ...
      projectFields[facetKey] = { $ifNull: [fieldAccess, 0] };
    } else {
      // For simple fields, use dot notation
  ...
  try {
    ...
    console.error(`Error executing historical badges aggregate:`, error);
