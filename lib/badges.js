...
const XP_BONUS = { cleanRound: 100, noOB: 50 };
...
/**
 * Marginal XP from verified / weekly multipliers (order matches how `multiplier` is built: +verified, then +weekly).
 */
...
/**
 * Per-player XP attribution for one round, aligned to XP_SCORE / XP_TIER / XP_BONUS / XP_MULTIPLIER.
 * `xpTier` is one entry from {@link XP_TIER}: the reward for the **highest tier index** achieved this round
 * among tiered badge grants, or tier 0 when only unique badges were earned (they use `XP_TIER[0]`).
 * `null` when no badge tier / unique XP was earned this round.
 */
function buildPlayersXpBreakdownEntry(
  ...
  for (const bo of badgeObjectsWithXP || []) {
    const grant = Number(bo.xp) || 0;
    if (grant <= 0) continue;
    const idx = Math.min(Math.max(Number(bo.currentTier) || 0, 0), XP_TIER.length - 1);
    if (idx > maxTierIdx) maxTierIdx = idx;
  }
  if (uniqueBadgesEarnedThisRound > 0) {
    maxTierIdx = Math.max(maxTierIdx, 0);
  }
  const xpTier = maxTierIdx >= 0 ? XP_TIER[maxTierIdx] : null;
  ...
  return {
    entityId: idObj,
    xpScore: { ...roundXPResult.xpByScoreType },
    xpTier,
    xpBonus: {
 ...
 * @returns {{ baseXP: number, hotStreakXP: number, cleanRound: boolean, noOB: boolean, xpByScoreType: Record<string, number>, breakdown: object }}
 */
...
/**
 * Get XP bonus flags: verified round, weekly play streak (2+ weeks).
 */
async function getXPBonuses(db, playerId, scorecard) {
  ...
  return { verifiedRound, weeklyStreak };
}
...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {  
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    let xpBreakdownRow = buildPlayersXpBreakdownEntry(
      ...
      { verifiedRound: false, weeklyStreak: false },
      [],
      0
    );
    try {
