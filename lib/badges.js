...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {  
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    for (const badge of earnedForPlayer) {
      docsToInsert.push({
        _id: deterministicObjectIdFromString(`${playerIdStr}-${badge.badgeId}`),
        userId: playerId,
  ...
  console.log('perPlayerEarned', perPlayerEarned);

  if (bulkOps.length > 0) {

    var { ok, result, writeErrors } = await bulkWriteWithOutcome(progressCollection, bulkOps);

    console.log('result, writeErrors', result, writeErrors);

    // remove the documents that have writeErrors from perPlayerEarned
    perPlayerEarned = perPlayerEarned.filter(p => !writeErrors.some(e => e.documentKey._id === p.userId));
