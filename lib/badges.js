...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout }) => {
  ...
  const uniquePlayerIds = Array.from(new Set(results.map(r => {
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    if (historicalBadges.length > 0) {
      ...
      try {
        const historicalResults = await checkHistoricalBadges(db, playerId, historicalBadges, courseId, scorecardId);
...
const checkHistoricalBadges = async (db, playerId, historicalBadges, courseId, scorecardId = null) => {
  ...
  console.log(`   Scorecard ID (to exclude): ${scorecardId}`);
  ...
  try {
    ...
    // DEBUG: Log the pipeline structure for Friendly Fire
    if (facets.friendly_fire) {
      console.log(`\n🔍 [DEBUG] Friendly Fire facet pipeline stages: ${facets.friendly_fire.length}`);
      if (facets.friendly_fire[0]) {
        console.log(`   First stage type: ${Object.keys(facets.friendly_fire[0])[0]}`);
      }
    }
    
    // DEBUG: Run a separate debug pipeline to inspect intermediate results
    const debugPipeline = [
      {
        $match: sharedMatch
      },
      ...
      },
      {
        $addFields: {
          friendIds: '$friends.friendId',
          debugInfo: {
            scorecardId: { $toString: '$_id' },
            creatorId: { $toString: '$creatorId' },
            resultsCount: { $size: { $ifNull: ['$results', []] } },
            friendsCount: { $size: { $ifNull: ['$friends', []] } },
            friendIdsCount: { $size: { $ifNull: ['$friends.friendId', []] } },
            firstFriendId: { $toString: { $arrayElemAt: ['$friends.friendId', 0] } },
            firstResultPlayerId: { $toString: { $arrayElemAt: ['$results.playerId', 0] } }
          }
        }
      },
      { $limit: 2 }
    ...
    console.log(`\n🔍 [DEBUG] Running debug pipeline to inspect data...`);
    const debugResult = await scorecardsCollection.aggregate(debugPipeline).toArray();
    console.log(`   Found ${debugResult.length} scorecard(s) in debug pipeline`);
    debugResult.forEach((doc, idx) => {
      console.log(`\n   Scorecard ${idx + 1}:`);
      console.log(`     Scorecard ID: ${doc.debugInfo?.scorecardId}`);
      console.log(`     Creator ID: ${doc.debugInfo?.creatorId}`);
      console.log(`     Results count: ${doc.debugInfo?.resultsCount}`);
      console.log(`     Friends found: ${doc.debugInfo?.friendsCount}`);
      console.log(`     FriendIds count: ${doc.debugInfo?.friendIdsCount}`);
      console.log(`     First friendId: ${doc.debugInfo?.firstFriendId}`);
      console.log(`     First result playerId: ${doc.debugInfo?.firstResultPlayerId}`);
      if (doc.friendIds && doc.friendIds.length > 0) {
        console.log(`     FriendIds array: ${doc.friendIds.map(id => id.toString()).join(', ')}`);
        console.log(`     FriendIds types: ${doc.friendIds.map(id => id.constructor.name).join(', ')}`);
      }
      if (doc.results && doc.results.length > 0) {
        const playerResults = doc.results.filter(r => r.playerId && r.playerId.equals && r.playerId.equals(playerIdObj));
        const friendResults = doc.results.filter(r => {
          if (!r.playerId || !doc.friendIds) return false;
          return doc.friendIds.some(fid => fid.equals && fid.equals(r.playerId));
        });
        console.log(`     Player results: ${playerResults.length}`);
        console.log(`     Friend results: ${friendResults.length}`);
        if (playerResults.length > 0 && friendResults.length > 0) {
          const playerTotal = playerResults.reduce((sum, r) => sum + (r.score || 0), 0);
          const friendTotals = {};
          friendResults.forEach(r => {
            const fid = r.playerId.toString();
            friendTotals[fid] = (friendTotals[fid] || 0) + (r.score || 0);
          });
          const bestFriendTotal = Math.min(...Object.values(friendTotals));
          console.log(`     Player total: ${playerTotal}`);
          console.log(`     Friend totals: ${JSON.stringify(friendTotals)}`);
          console.log(`     Best friend total: ${bestFriendTotal}`);
          console.log(`     Player lost? ${playerTotal > bestFriendTotal}`);
        }
      }
    });
