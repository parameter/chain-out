...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout }) => {
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    // Filter out badges with currentTier === -1, then sort by progress value (greatest first) and take top 3
    const eligibleBadges = badgeObjects.filter(badge => badge.currentTier !== -1);

    const sortedProgressBadges = [...eligibleBadges]
      .sort((a, b) => (b.progress || 0) - (a.progress || 0))
      .slice(0, 3);

    // Badges that progressed the most this round (not tier-based)
    const progressedMost = [...eligibleBadges]
      .filter(b => (b.progress || 0) > 0)
      .sort((a, b) => (b.progress || 0) - (a.progress || 0))
      .slice(0, 3);

    // Store progress badges (what we're inserting into userBadgeProgress)
    perPlayerEarned.push({
      userId: playerIdStr,
      badges: sortedProgressBadges,
      progressedMost,
      // Back-compat: keep old field name but now progress-based
      tierUps: progressedMost
    });
