...
const updateBadgeProgress = async (userId, progressData, badgeDefinitions, courseId, layoutId, scorecardId) => {
  ...
  if (existingProgress) {
    ...
    // Handle trackTierThresholdZync - check if progressData.progress matches a threshold
    let matchedThreshold = null;
    if (badgeDef && badgeDef.trackTierThresholdZync && badgeDef.tierThresholds) {
      // Find the highest threshold that is <= progressData.progress
      const validThresholds = badgeDef.tierThresholds.filter(threshold => threshold <= progressData.progress);
      
      if (validThresholds.length > 0) {
        matchedThreshold = Math.max(...validThresholds);
        
        // Add tierThresholds to document if not already present
        if (!existingProgress.tierThresholds) {
          updateOps.$set.tierThresholds = badgeDef.tierThresholds;
        }
      }
    }
    
    // Initialize $addToSet if needed for either trackUniqueCourses or trackTierThresholdZync
    if ((badgeDef && badgeDef.trackUniqueCourses) || matchedThreshold !== null) {
      updateOps.$addToSet = {};
    }
    ...
    // Add the matched threshold to trackedThresholds if found
    if (matchedThreshold !== null) {
      updateOps.$addToSet.trackedThresholds = matchedThreshold;
    }
    ...
    // Execute update query with conditional uniqueness checks
    let updateQuery = { userId: userId, badgeId: badgeDef._id };
    ...
    // Check trackedThresholds if this badge tracks tier threshold zync and we found a match
    if (badgeDef && badgeDef.trackTierThresholdZync && matchedThreshold !== null) {
      const trackedThresholds = existingProgress.trackedThresholds || [];
      
      // Check if the threshold is already tracked
      if (trackedThresholds.includes(matchedThreshold)) {
        console.log(`Threshold ${matchedThreshold} already tracked for badge ${progressData.badgeName}, skipping update`);
        return null;
      }
      
      // Add condition to query: only update if threshold is not in trackedThresholds
      updateQuery.trackedThresholds = { $nin: [matchedThreshold] };
    }
    ...
    // If no documents were updated and this badge tracks tier threshold zync, it means the threshold was already tracked
    if (updateResult.matchedCount === 0 && badgeDef && badgeDef.trackTierThresholdZync && matchedThreshold !== null) {
      console.log(`Threshold ${matchedThreshold} already tracked for badge ${progressData.badgeName}, skipping update`);
      return null;
    }
    ...
    // Check if progressData.progress matches a threshold for new progress
    let matchedThreshold = null;
    if (badgeDef && badgeDef.trackTierThresholdZync && badgeDef.tierThresholds) {
      // Find the highest threshold that is <= progressData.progress
      const validThresholds = badgeDef.tierThresholds.filter(threshold => threshold <= progressData.progress);
    ...
    newProgress = {
      ...
      trackedThresholds: (matchedThreshold !== null) ? [matchedThreshold] : [],
