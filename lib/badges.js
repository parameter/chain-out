...
const getHoleZone = (hole) => {
  return String(hole?.zone || hole?.region || hole?.tee || '').trim().toUpperCase();
};
...
const searchForEarnedAchievements = async ({ results, courseId, layout, scorecard }) => {
  ...
  for (const [playerId, playerResults] of playerResultsMap.entries()) {
    ...
    for (const ach of achievementDefs) {
      const selectedHoles = Array.isArray(ach.selectedHoles) ? ach.selectedHoles.map((n) => Number(n)).filter(Number.isFinite) : [];
      ...
      const expectedResult = normalizeResultValue(ach.result);
      if (expectedResult != null) {
        const resultMatches = relevantResults.every((r) => {
          const holeNumber = Number(r.holeNumber);
          const hole = holesByNumber.get(holeNumber);
          const par = getHolePar(hole);
          const score = Number(r.score);
          if (!Number.isFinite(score)) return false;
          return (score - par) === expectedResult;
        });
        if (!resultMatches) continue;
      }

      if (ach.zone) {
        const expectedZone = String(ach.zone).trim().toUpperCase();
        const zoneMatches = relevantResults.every((r) => {
          const hole = holesByNumber.get(Number(r.holeNumber));
          return getHoleZone(hole) === expectedZone;
        });
