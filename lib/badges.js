...
const searchForEarnedBadges = async ({ scorecardId, results, courseId, layout, scorecard }) => {  
  ...
  for (const playerIdObj of uniquePlayerIds) {
    ...
    if (progressBulkOps && progressBulkOps.length > 0) {
      addTotalXPToTierProgressOps(progressBulkOps, totalXP);
...
const addTotalXPToTierProgressOps = (ops, totalXP) => {
  if (!Array.isArray(ops) || typeof totalXP !== 'number') return;

  for (const op of ops) {
    const pipeline = op?.updateOne?.update;
    ...
    for (const stage of pipeline) {
      ...
      if (newTierProgressEntry && typeof newTierProgressEntry === 'object') {
        newTierProgressEntry.xp = totalXP;
      }
    }
  }
};
...
const fetchEarnedBadgesAndAchievementsForScorecard = async ({ scorecardId, results, courseId, layout, scorecard }) => {
  ...
  const badgesByEntity = badges.reduce((acc, badge) => {
    ...
    const latestTierProgress = tierProgress.length > 0 ? tierProgress[tierProgress.length - 1] : null;
    acc[entityId].push({
      ...badge,
      xp: latestTierProgress?.xp
    });
    return acc;
  }, {});
