  ...
  for (const [userId, playerResults] of playerResultsMap.entries()) {
    ...
    for (const ach of dedupedAchievementDefs) {
      ...
      if (
        shouldPersistProgress &&
        streakSatisfied &&
        !alreadyWonAchievement
      ) {
        const xpEarned = getXpForAchievementDifficulty(ach.difficulty);
        if (xpEarned > 0) {
...
const fetchEarnedBadgesAndAchievementsForScorecard = async ({ scorecardId, results, courseId, layout, scorecard }) => {
  ...
  // Denominator for userPercentage: total verified users in the users collection
  const totalVerifiedUsers = await usersCollection.countDocuments({ emailVerified: true });

  // userPercentage = % of verified users (from users collection) that have:
  //   - badges: a userBadgeProgress doc for the same badgeId at the same currentTier
  //   - achievements: a userAchievementProgress doc for the same achievementId with won: true
  const [badgeProgressDocs, achievementProgressDocs] = await Promise.all([
    badgesCollection.aggregate([
      ...
      },
      {
        $lookup: {
          ...
          let: { bid: '$badgeId', tier: '$currentTier' },
          pipeline: [
            {
              $match: {
                $expr: {
            ...
            // Dedupe by user so each user counts at most once
            { $group: { _id: '$userId' } },
            // Only count users that exist in the users collection with emailVerified: true
            {
              $lookup: {
                from: 'users',
                let: { uid: '$_id' },
      ...
      },
      {
        $addFields: {
          userPercentage: {
            $cond: [
              { $gt: [totalVerifiedUsers, 0] },
              {
                $multiply: [
                  {
                    $divide: [
                      { $ifNull: [{ $arrayElemAt: ['$badgeStats.holders', 0] }, 0] },
                      totalVerifiedUsers
                    ]
                  },
                  100
            ...
            $cond: [
              { $gt: [totalVerifiedUsers, 0] },
              {
                $multiply: [
                  {
                    $divide: [
                      { $ifNull: [{ $arrayElemAt: ['$achievementStats.holders', 0] }, 0] },
                      totalVerifiedUsers
                    ]
                  },
                  100
