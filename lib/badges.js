...
const fetchEarnedBadgesAndAchievementsForScorecard = async ({ scorecardId, results, courseId, layout, scorecard }) => {
  ...
  const progressDocs = await badgesCollection.find({
    $or: [
      { scorecardId: new ObjectId(scorecardId) },
      { 'tierProgress.scorecardId': new ObjectId(scorecardId) }
    ]
  }).toArray();

  console.log('fetchEarnedBadgesAndAchievementsForScorecard', progressDocs);

  const badgeDefinitions = await getBadgeDefinitions(db);
  const badgeDefById = new Map(
    badgeDefinitions.map((def) => [String(def.id), def])
  );
  const badges = progressDocs.map((badge) => {
    const def = badgeDefById.get(String(badge.badgeId));
    let nextThreshold = def.tierThresholds[badge.currentTier + 1];
    return {
      ...badge,
      tierThresholds: Array.isArray(def?.tierThresholds) ? def.tierThresholds : null,
      nextTierDescription: `${def.tierDescriptionPrefix} ${nextThreshold} ${def.tierDescriptionSuffix}`
    };
  });
  ...
  const badgesPerPLayer = Object.entries(badgesByEntity).map(([entityId, userBadges]) => ({
    entityId,
    badges: userBadges
  }));
