...
async function getXPBonuses(db, playerId, scorecard, roundFlags = {}) {
  ...
  const completedRounds = await scorecardsCollection
    .find({
      $or: [{ 'results.playerId': playerIdObj }, { 'results.entityId': playerIdObj }],
      status: 'completed'
    })
  ...
  if (cleanRoundThisRound && scorecardId != null) {
    const excludeId = scorecardId instanceof ObjectId ? scorecardId : new ObjectId(String(scorecardId));
    const prev = await scorecardsCollection.findOne(
      {
        status: 'completed',
        _id: { $ne: excludeId },
        $or: [{ 'results.playerId': playerIdObj }, { 'results.entityId': playerIdObj }]
      },
      { sort: { updatedAt: -1 }, projection: { results: 1, layout: 1, updatedAt: 1 } }
    );
    if (prev) {
